...
public class ServerInfoControllerTest {
    ...
    void t03_accessDetail() throws Exception {
        ...
        //serverInfoService.findAllServerInfo() 호출 발생시 반환값 설정
        given(serverInfoService.findAllServerInfo()).willReturn(serverInfoList);
        given(serverInfoService.findServerInfoById(serverId)).willReturn(serverInfo);
        given(diskService.findAllDisksByServerId(serverId)).willReturn(diskList);
        //http.get 요청
        mvc.perform(post("/detail/{serverId}", serverId))
                ...
                .andExpect(model().attributeExists("detailDto"))
                //detaildto라는 속성에 각 필드가 있는지, get으로 가져온 데이터와 일치하는지 확인
                .andExpect(model().attribute("detailDto", hasProperty("ip", is(detailDto.getIp()))))
                .andExpect(model().attribute("detailDto", hasProperty("memoryTotal", is(detailDto.getMemoryTotal()))))
                .andExpect(model().attribute("detailDto", hasProperty("diskId", is(detailDto.getDiskId()))))
                .andExpect(model().attribute("detailDto", hasProperty("diskName", is(detailDto.getDiskName()))))
                .andExpect(model().attribute("detailDto", hasProperty("diskTotal", is(detailDto.getDiskTotal()))));
    }
}
