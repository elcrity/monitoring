...
const Dashboard = () => {
    ...
    if (errorMessage != null) { return <div>loading</div> }
    return (
        //전체
        <div id='container'>
            <div id='header'>
            ...
            <div id="serverTableBody">
                {mergedData.length > 0 ? (
                    mergedData.map((mergedData) => (
                        <ServerBoard key={mergedData.serverId} data={mergedData} onDiskClick={handleDiskClick} onDeleteClick={handleDeleteClick} />
                    ))
                ) : (
                    <div>
